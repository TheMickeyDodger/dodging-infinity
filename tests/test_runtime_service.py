...
class CliTests(Fixture):
    ...
    def test_default_root_from_this_worktree_is_refused_and_writes_nothing(self):
        ...
        # launchctl call. No --root is given, so the CLI falls back to
        # cli._repository_root(); that is patched to a fixture root
        # that is unstable BY CONSTRUCTION (a .git FILE, i.e. a git
        # worktree), so the refusal does not depend on whether the
        # checkout running the suite is itself stable (CI) or not
        # (an engineering worktree). Nothing is written, nothing is
        # issued.
        worktree = self.make_root("worktree")
        os.rmdir(os.path.join(worktree, ".git"))
        with open(os.path.join(worktree, ".git"), "w") as handle:
        ...
        with patch.dict(os.environ, {"HOME": self.home}), patch.object(
            cli, "_repository_root", return_value=worktree
        ), patch.object(
